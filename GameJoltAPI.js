...
GJAPI.SendRequestEx = function (b, a, c, d, e) { b = GJAPI.sAPI + encodeURI(b) + (-1 === b.indexOf("/?") ? "?" : "&") + "game_id=" + GJAPI.iGameID + "&format=" + c; GJAPI.bLoggedIn && a === GJAPI.SEND_FOR_USER && (b += "&username=" + GJAPI.sUserName + "&user_token=" + GJAPI.sUserToken); if (d) { b += "&signature=" + hex_md5(b + GJAPI.sGameKey).replace('>',''); } else { b += "&signature=" + hex_md5(b + GJAPI.sGameKey); } __CreateAjax(b,d,function(f){console.info(GJAPI.sLogName+" <"+b+"> "+f);if(""!==f&&"function"===typeof e)switch(c){case "json":e(eval("("+f+")").response);break;case "dump":var g=f.indexOf("\n"),h=f.substr(0,g-1);f=
...
class GameJoltAPI {
    ...
    dataStoreFetch(args) {
        GJAPI.DataStoreFetch(args.globalOrPerUser, args.key, function (pResponse) {
            if (!pResponse.success) { err.store = pResponse.data; return; }
