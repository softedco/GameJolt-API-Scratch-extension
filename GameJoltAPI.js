...
class GameJoltAPI {
    ...
    returnUserData(args) {
        switch(args.userDataType) {
            case 'username':
                return userData.users[0]['username'];
            case 'ID':
                return userData.users[0]['id'];
            case 'status':
                return userData.users[0]['status'];
            case 'profile description':
                return userData.users[0]['developer_description'];
            case 'sign up date':
                return userData.users[0]['signed_up'];
            case 'last login':
                return userData.users[0]['last_logged_in'];
            default:
                return err;
        };
    }
