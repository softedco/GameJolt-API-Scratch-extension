...
class GameJoltAPI {
    getInfo() {
        return {
            ...
            blocks: [
                ...
                {
                    opcode: 'trophyIDFetch',
                    blockIconURI: trophyIcon,
                    blockType: Scratch.BlockType.REPORTER,
                    text: 'Fetch [trophyType] trophy ID by index:[index]',
                    arguments: {
                        trophyType: {
                            type: Scratch.ArgumentType.STRING,
                            menu: 'trophyTypes',
    ...
    returnUserData(args) {
        ...
        switch(args.userDataType) {
            ...
            case 'names':
                return Object.getOwnPropertyNames(userData);
            default:
                return err;
        };
    }
        ...
        switch(args.indexOrID) {
            case 'index':
                ...
                switch(args.trophyDataType) {
                    ...
                    case 'names':
                        return Object.getOwnPropertyNames(trophyData[args.value]);
                    default:
                        trophyData = err;
                        return err;
                }
            case 'ID':
                ...
                switch(args.trophyDataType) {
                    ...
                    case 'names':
                        return Object.getOwnPropertyNames(trophyData);
                    default:
                        trophyData = err;
                        return err;
                }
    ...
    trophyIDFetch(args) {
        switch(args.trophyType) {
            case 'achieved':
                GJAPI.TrophyFetch(GJAPI.TROPHY_ONLY_ACHIEVED, function (pResponse) {
                    if (!pResponse.trophies) { achievedData = err; return err; }
                    achievedData = pResponse.trophies;
                });
                if (typeof achievedData != 'object') { return err; }
                if (typeof achievedData[args.index] != 'object') { return err; }
                achievedData[args.index].id = achievedData[args.index].id ?? err;
                return achievedData[args.index].id;
            case 'not achieved':
                GJAPI.TrophyFetch(GJAPI.TROPHY_ONLY_NOTACHIEVED, function (pResponse) {
                    if (!pResponse.trophies) { notAchievedData = err; return err; }
                    notAchievedData = pResponse.trophies;
                });
                if (typeof notAchievedData != 'object') { return err; }
                if (typeof notAchievedData[args.index] != 'object') { return err; }
                notAchievedData[args.index].id = notAchievedData[args.index].id ?? err;
                return notAchievedData[args.index].id;
            case 'any':
                GJAPI.TrophyFetch(GJAPI.TROPHY_ALL, function (pResponse) {
                    if (!pResponse.trophies) { trophyData = err; return err; }
                    trophyData = pResponse.trophies;
                ...
                if (typeof trophyData != 'object') { return err; }
                if (typeof trophyData[args.index] != 'object') { return err; }
                trophyData[args.index].id = trophyData[args.index].id ?? err;
                return trophyData[args.index].id;
            default:
                return err;
        }
    }
    ...
    returnScoreData(args) {
        ...
        switch(args.scoreDataType) {
            ...
            case 'name':
                return Object.getOwnPropertyNames(scoreData[args.index]);
            default:
                return err;
        }
    }
