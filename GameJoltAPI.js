...
GJAPI.FETCH_FOR_USER = true;
GJAPI.FETCH_FOR_GUEST = false;
...
GJAPI.ScoreFetchUserEx = function (iScoreTableID, bUserOrGuest, bName, iLimit, iBetterOrWorse, iValue, pCallback) {
    GJAPI.SendRequest("/scores/" +
                      "?limit=" + iLimit +
                      (iScoreTableID ? ("&table_id=" + iScoreTableID) : "") +
                      (bUserOrGuest ? '&username=' : '&guest=') + bName +
                      (iBetterOrWorse ? '&better_than=' : '&worse_than=') + iValue,
...
class GameJoltAPI {
    ...
    getInfo() {
        return {
            ...
            blocks: [
                ...
                {
                    opcode: 'scoreFetchUser',
                    blockIconURI: scoreIcon,
                    blockType: Scratch.BlockType.COMMAND,
                    text: 'Fetch [amount][userOrGuest][username] score/scores [betterOrWorse] than [value] by ID:[ID]',
                    arguments: {
                        amount: {
                            type: Scratch.ArgumentType.NUMBER,
                            defaultValue: 1
    ...
    friendsFetch(args) {
        ...
        if (typeof friendsData[args.index] != 'object') { return err; }
        friendsData[args.index].friend_id = friendsData[args.index].friend_id ?? err;
        return friendsData[args.index].friend_id;
    }
    trophyAchieve(args) {
    ...
    scoreFetch(args) {
        switch (args.globalOrPerUser) {
            case 'global':
                switch (args.betterOrWorse) {
                    case 'better':
                        GJAPI.ScoreFetchEx(args.ID, GJAPI.SCORE_ALL, args.amount, GJAPI.BETTER_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
                    case 'worse':
                        GJAPI.ScoreFetchEx(args.ID, GJAPI.SCORE_ALL, args.amount, GJAPI.WORSE_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
            ...
            case 'user':
                switch (args.betterOrWorse) {
                    case 'better':
                        GJAPI.ScoreFetchEx(args.ID, GJAPI.SCORE_ONLY_USER, args.amount, GJAPI.BETTER_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
                    case 'worse':
                        GJAPI.ScoreFetchEx(args.ID, GJAPI.SCORE_ONLY_USER, args.amount, GJAPI.WORSE_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
    ...
    scoreFetchUser(args) {
        switch (args.userOrGuest) {
            case 'user':
                switch (args.betterOrWorse) {
                    case 'better':
                        GJAPI.ScoreFetchUserEx(args.ID, GJAPI.FETCH_FOR_USER, args.username, args.amount, GJAPI.BETTER_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
                    case 'worse':
                        GJAPI.ScoreFetchUserEx(args.ID, GJAPI.FETCH_FOR_USER, args.username, args.amount, GJAPI.WORSE_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
            ...
            case 'guest':
                switch (args.betterOrWorse) {
                    case 'better':
                        GJAPI.ScoreFetchUserEx(args.ID, GJAPI.FETCH_FOR_GUEST, args.username, args.amount, GJAPI.BETTER_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
                    case 'worse':
                        GJAPI.ScoreFetchUserEx(args.ID, GJAPI.FETCH_FOR_GUEST, args.username, args.amount, GJAPI.WORSE_THAN, args.value, function (pResponse) {
                            if (!pResponse.scores) { scoreData = err; return; }
                            scoreData = pResponse.scores;
                        });
                        break;
    ...
    scoreGetTables(args) {
        GJAPI.ScoreGetTables( function (pResponse) {
            if (pResponse.success == f) { tablesData = err; return; }
            tablesData = pResponse.tables;
        });
